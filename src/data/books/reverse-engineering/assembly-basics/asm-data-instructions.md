...
![x86 双操作数指令: 操作数组合规则 (允许: 寄存器<-立即数/寄存器/内存, 内存<-立即数/寄存器; 禁止: 内存<-内存, 立即数做目的操作数)](asm-data-instructions-images/operand-rules.png)
...
它的主要用途就是"占位"和"擦除"。第1章你用 <kbd>Ctrl</kbd>+<kbd>9</kbd> 把 `jne` 填充成 NOP，就是用无害的空操作替换掉跳转指令，让那条判断失效。
