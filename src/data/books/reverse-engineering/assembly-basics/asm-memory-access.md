...
前面我们看到的地址，有的形如 `[ebp-4]`（栈上），有的形如 `ds:[0x0040A000]`（固定地址）。这些地址指向哪里？它们在进程内存中的位置关系是怎样的？

一个 32 位程序运行时，操作系统给它分配一块**虚拟地址空间**（0 到 0xFFFFFFFF），按功能分成几个区域：

![32 位进程内存布局：栈在高地址向下生长，堆在中间，.data/.bss/.rdata 段存全局变量和只读数据，.text 段存代码](asm-memory-access-images/memory-layout.png)

几个要点：

- **栈（Stack）** 在高地址，往低地址生长。`[ebp-X]` 访问的就是栈上的局部变量
- **堆（Heap）** 在栈下方，往高地址生长。`malloc`/`new` 分配的内存在这里，地址不固定
- **.data 段** 存有初始值的全局变量，地址固定（如 `0x0040A000`）
- **.bss 段** 存未初始化的全局变量，运行时自动清零
- **.rdata 段** 存只读数据：`const` 常量、字符串字面量（如 `"Hello"`）、导入表
- **.text 段** 存编译后的机器码，`0x00401000` 就在这里

> [!NOTE] 为什么内存要分段？
> 因为 **CPU 的内存保护是按页（4KB）为粒度的**，操作系统需要给不同数据设不同的权限：
>
> - 代码（`.text`）需要**可执行**（ER），但不能写，防止程序运行时被篡改
> - 全局变量（`.data`）需要**可读写**（RW），但不能执行，防止数据区被注入代码
> - 常量（`.rdata`）只需要**只读**（R），写入会触发异常
>
> 如果全部混在一起，要么全可执行（危险），要么全不可执行（代码跑不了）。分段让 OS 能对每一段设精确的内存保护。
>
> 你在 x64dbg 的内存地图里还能看到 `.rsrc`（资源：图标、对话框）、`.reloc`（ASLR 重定位表）等，它们有各自的专门用途。这些段的具体细节会在后面 PE 文件结构章节展开。

逆向时看到 `ds:[0x004XXXXX]` 形式的地址，基本就是 .data/.bss/.rdata 段里的全局变量。后面讲函数调用时会经常对比这两种地址风格。

> [!NOTE] x64dbg 里的段前缀
> 在 x64dbg 里你会看到内存地址前面带着 `ss:` 或 `ds:` 等前缀：
>
> - `ss:[ebp-8]` — 栈段（Stack Segment），访问局部变量时出现
> - `ds:[0x0040A000]` — 数据段（Data Segment），访问全局变量时出现
> - 偶尔还有 `fs:[0]`（异常处理）、`es:[edi]`（串指令目标）、`gs:[0]`（64 位 TLS）
>
> 这是 x86 段寄存器的遗留产物。32 位平坦内存模型下，`ss` 和 `ds` 指向同一段，实际没有区别。逆向时可以忽略段前缀，直接看方括号里的地址。
...
> [!TIP]
> 在 x64dbg 里怎么找到那片 NOP 区域？程序加载后会停在系统断点，按 <kbd>Alt</kbd>+<kbd>F9</kbd>（执行到用户代码）就能跳到你的 NOP 区域。按 **空格键**，输入你想实验的指令（比如 `mov eax, 0x12345678`），回车确认。光标自动移到下一行，你可以继续输入下一条。输入完毕后在第一条按 <kbd>F2</kbd> 设断点，<kbd>F9</kbd> 运行到这里，然后 <kbd>F8</kbd> 单步执行，观察寄存器和标志位的变化。
