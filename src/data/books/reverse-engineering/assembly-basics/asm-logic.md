...
上一章学了算术指令，它们做"数学运算"。这一章学**逻辑指令**和**移位指令**，它们做"位运算"，包括 `and`、`or`、`xor`、`not` 四条逻辑指令和 `shl`、`shr`、`rol`、`ror` 四条移位指令。
...
逐位做与运算：两位都是 1，结果才是 1；否则为 0。最常用的场景是**掩码操作**，用 `and` 把不需要的位清零。操作数规则同第 6 章的 `add`。
...
逐位做异或运算：两位不同为 1，相同为 0。`xor` 最常见的用法是**清零**，一个数和自己异或，结果一定是 0。操作数规则同第 6 章的 `add`。
...
> [!NOTE] 编译器为什么要优化？
> 位运算指令通常比等价的算术指令**更短、更快**。`xor eax, eax` 清零只要 2 字节，`mov eax, 0` 要 5 字节。后面会看到 `shl eax, 1` 乘 2 也比 `imul` 更短更快。更短的指令意味着更小的程序体积、更少的内存访问、更快的执行速度。编译器在生成代码时会自动选择最优的指令组合，所以你看到的汇编代码往往和源代码不是一一对应的。
...
`sal` 和 `shl` 是同一条指令，关键区别在 `sar`，它处理有符号数的右移：
...
![rol 循环左移追踪](asm-logic-images/rol-trace.png)

再看 `rol eax, 4`，同样的初始值：

![rol 循环左移 4 位追踪](asm-logic-images/rol4-trace.png)
...
![ror 循环右移 4 位追踪](asm-logic-images/ror4-trace.png)
