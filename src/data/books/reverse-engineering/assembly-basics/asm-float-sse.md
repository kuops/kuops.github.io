...
> [!NOTE] double（双精度）怎么拆
> 上面讲的是 32 位 `float`（单精度）。C 里还有 `double`（双精度），64 位，规则完全一样，只是每段更宽：
>
> |        | float (32 位)  | double (64 位)   |
> | ------ | -------------- | ---------------- |
> | 符号位 | 1 位           | 1 位             |
> | 指数位 | 8 位, 偏移 127 | 11 位, 偏移 1023 |
> | 尾数位 | 23 位          | 52 位            |
>
> 指数偏移量从 127 变成 1023，尾数从 23 位变 52 位，公式形式 `V = (-1)^S × 1.M × 2^(E - 偏移)` 不变。double 精度更高，但逆向时 32 位 float 更常见（游戏坐标、血量大多用 float）。
