...
![x64dbg 四窗口布局示意](first-crack-images/x64dbg-four-panes-2.png)
...
![x64dbg 四窗口布局](first-crack-images/x64dbg-four-panes.png)
...
> **找不到 "Correct!" 怎么办？** 如果字符串列表里没有 "Correct!"，检查以下几点：
>
> 1. **编译模式是 Debug 还是 Release？** Release 模式下编译器可能优化掉字符串或内联代码。确保用 **Debug** 模式编译
> 2. **平台是 x86 吗？** 如果编译成了 x64，要用 x64dbg.exe（不是 x32dbg.exe）打开，字符串搜索步骤一样
> 3. **搜 "Wrong" 试试** — 有时 "Correct!" 和 "Wrong!" 不在同一个列表里，搜另一个
> 4. **重新加载** — 把 exe 重新拖进 x32dbg，再搜一次。有时候操作顺序不对会漏掉
...
> **小知识：jne 和 jnz 是同一条指令**
>
> `jne`（Jump if Not Equal）和 `jnz`（Jump if Not Zero）其实是同一条机器码，只是两种不同的助记符写法。零标志位 ZF=0 意味着"结果不为零"也就是"不相等"，所以它们完全等价。x64dbg 里可能显示 `jne` 也可能显示 `jnz`，取决于上下文——你看多了就习惯了。同理，`je`（Jump if Equal）和 `jz`（Jump if Zero）也是同一条指令。

操作方法：

1. 点击选中 `jne` 那行
2. 按**空格键**，弹出汇编编辑框（里面显示的是当前的 `jne` 指令）
...
> **恢复原始字节**：如果改完后想恢复，在改过的那行按 **Ctrl+Z**（撤销修改）。如果撤销不了，直接把 exe 重新拖进 x32dbg 就行——磁盘上的 exe 没有被修改，x64dbg 的修改只在内存里生效。
