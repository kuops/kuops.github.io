...
你可能好奇：`01000000` 怎么就是 1？因为 x86 CPU 用**小端序（Little Endian）**存数据——低位字节放在前面，高位字节放在后面。数值 `1` 的 32 位十六进制是 `0x00000001`，按小端序从低到高排列就是 `01 00 00 00`。x64dbg 按地址顺序显示，所以你看到的是 `01 00 00 00`，从右往左读回去就是 `00000001` = 1。后面的章节会正式讲这个概念，现在只要知道"字节顺序是反的"就行。

机器码和指令说的是同一件事——`B8 01000000` 就是 `mov eax, 1`。第1章你用 `Ctrl+9` 把 `jne` 填充成 NOP，其实就是把机器码 `75 xx` 改成了 `90 90`（`90` 就是 NOP 的机器码）。
...
你可能好奇为什么地址经常从 `0x00401000` 开始——这是 Windows 32 位 PE 程序的**默认加载基址** `0x00400000`，代码从基址偏移 `0x1000` 处开始（前面是 PE 文件头）。所以你看到的第一条指令几乎总是在 `0x00401xxx` 附近。如果程序开了 ASLR 随机化，基址会变，但相对偏移不变。

你可以按 **Alt+M** 打开 Memory Map 窗口，看到当前程序实际占用了哪些内存区域。你会看到类似这样的一行行列表：

```
地址          大小      类型        内容
00AA0000   00003000   用户模块    l_intl.nls（系统文件）
00C70000   00001000   用户模块    你的程序.exe  ← 找这行
00C71000   00002000   用户模块    你的程序.exe
73E50000   000XX000   系统模块    MSVCRxxx.dll
773D0000   00123000   系统模块    kernel32.dll
...
```

你实际看到的地址、文件名都跟我的不一样——这取决于 Windows 版本、程序大小、加载的 DLL。**不用记具体地址**，只要记住几个概念：

- **找到你的 exe**：在"内容"列里找你的程序名（比如 `first-crack.exe`），那几行就是你程序的代码和数据
...
第三行就是第1章你用过的 NOP——`0x90` 当代码看是空操作，当数据看就是数字 144。
...
你可能注意到：每次重新打开 x64dbg 加载同一个程序，指令的地址都是一样的（比如都是从 `00401000` 开始）。但如果你**改了代码重新编译**，地址可能就变了——因为代码长度变了，链接器重新安排了位置。

另外，如果你开了 Windows 的 ASLR（地址空间布局随机化），每次运行的基地址会随机偏移。但简单的小程序（像我们的 CrackMe）一般不启用 ASLR，地址是固定的。
...
- **EAX** — 函数的返回值**总是**放在 EAX 里。调用一个函数后，看 EAX 就知道返回了什么。第1章 `strcmp` 的返回值就在 EAX 里
- **ECX** — 循环的计数器。你看到 `dec ecx` + `jne`（Jump if Not Equal，后面会讲），多半是个循环
- **EBP / ESP** — 这两个定义了当前函数的"栈帧"（后面第7章详细讲）。简单说：`[ebp-4]`、`[ebp-8]` 就是局部变量，ESP 始终指向栈顶
...
你可能注意到：x64dbg 的标题栏有时显示"x32dbg"。是不是装错了？

**没有装错。** x64dbg 是一个调试器套件，同时支持 32 位和 64 位程序。当你打开一个 32 位 exe 时，它自动以 x32dbg 模式运行；打开 64 位 exe 时，才是真正的 x64dbg 模式。

本书示例使用的 CrackMe 是 32 位程序，所以你看到的都是 x32dbg 模式——寄存器是 EAX、EBX 这些 32 位名字。

64 位模式的区别主要在：

- 寄存器从 EAX/EBX/... 扩展为 64 位的 RAX/RBX/...，外加新增 R8-R15 八个寄存器
- EIP 变成 RIP（64 位指令指针）
