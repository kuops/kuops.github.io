...
; int i = 100000
mov  dword ptr [ebp-C], 0x186A0       ; 0x186A0 = 100000, dword = 4 字节

; float f = 3.14f
mov  dword ptr [ebp-10], 0x4048F5C3   ; 3.14 的 IEEE 754 编码, dword

; long long ll = 100
mov  dword ptr [ebp-18], 0x64         ; 低 4 字节 = 100 (小端序, 低地址)
mov  dword ptr [ebp-14], 0x0          ; 高 4 字节 = 0

; double d = 3.14
mov  dword ptr [ebp-20], 0x51EB851F   ; 低 4 字节 (小端序, 低地址)
mov  dword ptr [ebp-1C], 0x40091EB8   ; 高 4 字节 (IEEE 754 双精度编码)
```
...
```asm
movss  xmm0, dword ptr [ebp-10]  ; 把 float 加载到 XMM0
addss  xmm0, dword ptr [ebp-14]  ; 浮点加法
movss  dword ptr [ebp-18], xmm0  ; 存回内存
```
...
push ebp
mov  ebp, esp
sub  esp, 0xE4                         ; Debug 模式分配大量栈空间
push ebx
push esi
push edi
lea  edi, dword ptr ss:[ebp-24]
mov  ecx, 9
mov  eax, 0xCCCCCCCC
...
mov  dword ptr ss:[ebp-14], 0x14      ; b = 20
mov  eax, dword ptr ss:[ebp-8]         ; eax = a
add  eax, dword ptr ss:[ebp-14]       ; eax = a + b
mov  dword ptr ss:[ebp-20], eax       ; c = a + b
mov  eax, dword ptr ss:[ebp-20]       ; 返回值
...
>
> - **`sub esp, 0xE4`**：分配 228 字节而非 12 字节。多出来的空间用于栈溢出检测。
...
mov  dword ptr [ebp-14], 0x14      ; b = 20
mov  eax, [ebp-8]                  ; 读 a
add  eax, [ebp-14]                ; a + b
mov  [ebp-20], eax               ; c = 结果
mov  eax, [ebp-20]               ; 返回值放 eax
```

**规律：局部变量 = `[ebp - X]`**。编译器把 `a`、`b`、`c` 这些名字翻译成了 `[ebp-8]`、`[ebp-14]`、`[ebp-20]` 这些栈偏移地址。
