...
1. **c-asm-10 结构体** — 旧草稿重构（order:20），按前 9 章风格重写
2. **asm-memory.md 审查** — 确认定位和内容完整度（order:21）
3. **c-asm-11 枚举与类型转换** — 新增章
4. **c-asm-12 位域与联合体** — 新增章
5. **c-asm-15 C++ this 指针** — 最大缺口，游戏逆向门槛
6. **c-asm-16 虚函数表** — 紧接 this，C++ 逆向核心
7. **c-asm-14 x86-64** — 补全 64 位
8. **cracking-1/2** — 破解篇地基（方法论 + PE）
9. **game-1 CE 入门** — 游戏篇起点
10. 其余按篇内顺序推进
