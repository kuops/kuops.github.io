...
适用于调试、逆向、网络分析、性能排查等需要多个工具或多个观察阶段的教程。审查时不要只确认“内容都写了”，还要确认读者能否沿着证据自行得到结论。

#### 用同一个问题贯穿多工具章节

- [ ] 开头用一句话说明整章始终追踪的对象或问题，而不是按工具名称罗列流程
- [ ] 明确每个工具负责回答什么，以及上一个工具留下了什么问题
- [ ] 切换工具前先收拢当前阶段的结论，再说明为什么下一个工具能够继续回答
- [ ] 不把章节写成“打开工具 A → 打开工具 B → 打开工具 C”的操作流水账
...
- [ ] 不在操作前泄露本应由操作发现的结果；先展示窗口、字段、字节或运行结果，再下结论
- [ ] 公式中的每个数字都已在前文说明来源，没有先参与计算、后读取字段的倒序
- [ ] 明确区分固定规范长度、文件字段给出的值、计算结果和运行时观察值
- [ ] 二进制推导尽量遵循“原始字节 → 字节序读取 → 字段含义 → 参与计算 → 得出结论”
- [ ] 静态结论需要动态验证时，已展示两边可直接比较的地址、机器码或数据，而不是只写“结果一致”

#### 检查每个术语的第一次出现

- [ ] 缩写首次出现时给出全称和本章语境下的含义
- [ ] 新窗口首次出现时先解释它回答什么问题，再给出菜单或快捷键
- [ ] 工具界面中的列只解释实际需要关注的部分；低频列放在折叠补充中
- [ ] 相似概念已明确区分，例如 Segment、PE Section、内存页，或文件偏移、RVA、VA
- [ ] 同一个名称在不同工具中记号不同时已做对照，例如 IDA 的 `R-X` 与 x32dbg 的 `ER-`

#### 将审稿对话重新融入正文

- [ ] 不把用户追问的回答原样追加到正文；修改后已重新阅读上下段
- [ ] 当前任务必需的解释放在正文，边界情况、高阶原理和排错细节放在折叠 `NOTE`
- [ ] 新增补充没有抢走当前小节的中心任务，也没有造成连续多个 callout
- [ ] 补充内容涉及后文概念时，只给当前理解所需的最小定义，并在后文正式展开

#### 用怀疑型新手视角终审

逐段停下来问：

- [ ] “这是什么？”——是否出现未定义的对象、字段、窗口、权限或缩写
- [ ] “为什么现在讲它？”——这段内容是否服务当前任务
- [ ] “这个数字从哪里来？”——能否在前文找到字段、规范或运行结果
- [ ] “上一个任务做完了吗？”——切换小节或工具前是否已有阶段结论
- [ ] “这是观察结果还是作者直接宣布的？”——关键结论是否有读者可见的证据
- [ ] “为什么突然换工具？”——过渡是否说明了新工具要解决的遗留问题
- [ ] “读完我能否自己推出同样结论？”——练习是否覆盖章节主线，而不只考旁支知识
